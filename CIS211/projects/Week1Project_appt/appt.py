"""
CIS 211: Week 1 Project Appointment

Author: Luke Vandecasteele

Credits: class notes, textbook, my dad

This file contains the classes Appt,
which will record the dates and times
that different events begin and end,
and Agenda, which sorts a list of
Appt class events.
"""
...
class Appt:
    ...
    def __lt__(self, other: 'Appt') -> bool:
        """Less than means self time period finishes before
        or at the same time that other time period starts"""
        return self.finish <= other.start

    def __gt__(self, other: 'Appt') -> bool:
        """Greater than means self time period starts
        after or at the same time other time period finishes """
...
class Agenda:
    ...
    def append(self, appointment: Appt) -> None:
    ...
    def conflicts(self) -> 'Agenda':
        ...
        ag_conflicts = Agenda()
        self.sort()
        num_items = self.elements.__len__()
        for item in range(num_items - 1):
            i = item + 1
            while(i <= num_items - 1):
                if self.elements[item].overlaps(self.elements[i]):
                    ag_conflicts.append(self.elements[item].intersect(self.elements[i]))
                    i = i + 1
                else:
                    break
        return ag_conflicts
