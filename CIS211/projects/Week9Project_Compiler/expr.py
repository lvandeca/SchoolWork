...
class Expr(object):
    ...
    def gen(self):
        raise NotImplementedError("gen should be implemented in subclasses")
...
class If(Control):
    ...
    def gen(self, context: Context, target: str):
        """If else"""
        loop_head_else = context.new_label("else")
        loop_exit = context.new_label("fi")
        self.cond.condjump(context, target, loop_head_else, jump_cond=False)
        self.thenpart.gen(context, target)
        context.add_line(f"   JUMP  {loop_exit}")
        context.add_line(f"{loop_head_else}:")
