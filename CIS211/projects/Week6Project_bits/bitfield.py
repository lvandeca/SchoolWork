"""
CIS 211 Bitfield packing and
unpacking

Author: Luke Vandecasteele
Credits: Class notes

Basic bit encoding and decoding functions
for packing bytes.
"""
...
class BitField(object):
    ...
    def __init__(self, from_bit: int, to_bit: int) -> None:
        ...
        self.to_bit = to_bit
        self.bit_range = (self.to_bit - self.from_bit) + 1
        self.mask = 0
        for bit in range(self.bit_range):
    ...
    def extract(self, word: int) -> int:
        ...
        """
        return self.mask & (word >> self.from_bit)
    ...
    def insert(self, value: int, word: int) -> int:
        """Insert value, which should be in the low order
         ...
         """
        trim = value & self.mask
        return (trim << self.from_bit) | word

    def extract_signed(self, word: int) -> int:
        """Extract bits in bitfield as a signed integer."""
        extract = self.mask & (word >> self.from_bit)
        return sign_extend(extract, self.bit_range)
