"""
CIS 211: Calculator Expression Project

Author: Luke Vandecasteele

Credits: Class notes, HOWTO

Post-fix calculator in python.
"""
...
class IntConst(Expr):
    ...
    def __eq__(self, other: Expr) -> bool:
        """ checks if two instances of IntConst are equal"""
...
class BinOp(Expr):
    ...
    def __str__(self) -> str:
        return f"({str(self.left)} {self.op_sym} {str(self.right)})" # added the str() inside {} check later if something wrong

    def __repr__(self) -> str:
        return f"{self.op_name}({repr(self.left)}, {repr(self.right)})" # same as for __str__ method
...
class UnOp(Expr):
    ...
    def __str__(self) -> str:
        return f"{self.op_sym} {str(self.left)}"  # added the str() inside {} check later if something wrong

    def __repr__(self) -> str:
        return f"{self.op_name}({repr(self.left)})"  # same as for __str__ method
