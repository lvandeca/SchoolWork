...
class Instruction(object):
    ...
    def encode(self) -> int:
        ...
        return_bit = 0b00000000000000000000000000000000
        return_bit = offset_field.insert(self.offset, return_bit)
        return_bit = reg_src2_field.insert(self.reg_src2, return_bit)
        return_bit = reg_src1_field.insert(self.reg_src1, return_bit)
        return_bit = reg_target_field.insert(self.reg_target, return_bit)
        return_bit = cond_field.insert(self.cond.value, return_bit)
        return_bit = instr_field.insert(self.op.value, return_bit)
        return return_bit
...
def decode(word: int) -> Instruction:
        ...
        temp_cond = cond_field.extract(word)
        temp_op = instr_field.extract(word)
        return Instruction(OpCode(temp_op), CondFlag(temp_cond), reg_target, reg_src1,
                           reg_src2, offset)